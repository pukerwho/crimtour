...
//Custom meta
function custom_meta_site($post_id, $meta) {
  $rating_array = ['meta_site_rating_user', 'meta_site_rating_our'];
  $big_array = ['meta_site_indexpages', 'meta_site_backlinks'];
  $visits_array = ['meta_site_visits'];
  $cms_array_meta = ['meta_site_cms'];
  $cms_array_value = ['WordPress', 'Tilda', 'OpenCart', 'Prestashop', 'Prom', 'Хорошоп', 'Joomla', 'Самописна'];
  $stat_array_meta = ['meta_site_stat'];
  $stat_array_value = ['Google Analytics', 'Яндекс Метрика', 'Liveinternet', 'Facebook Pixel'];

  if ( metadata_exists( 'post', $post_id, $meta ) ) {
    $meta_site = get_post_meta( $post_id, $meta, true );
    return $meta_site;
  } else {
    if (in_array($meta, $rating_array)) {
      $rand_value = mt_rand(2, 5);
    } elseif (in_array($meta, $big_array)) {
      $rand_value = mt_rand(20, 200);
    } elseif (in_array($meta, $visits_array)) {
      $rand_value = mt_rand(1, 100);
    } elseif (in_array($meta, $cms_array_meta)) {
      $rand_index = array_rand($cms_array_value, 1);
      $rand_value = $cms_array_value[$rand_index];
    } elseif (in_array($meta, $stat_array_meta)) {
      $rand_index = array_rand($stat_array_value, 1);
      $rand_value = $stat_array_value[$rand_index];
    } else {
      $rand_value = mt_rand(1, 8);
    }
    add_post_meta( $post_id, $meta, $rand_value, true );
    $meta_site = get_post_meta( $post_id, $meta, true );
    return $meta_site;
  }
}
