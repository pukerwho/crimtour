...
function tutCount($post_id) {
  if ( metadata_exists( 'post', $post_id, 'item_count' ) ) {
    $count_value = get_post_meta( $post_id, 'item_count', true );
    $count_value = $count_value + 1;
    update_post_meta( $post_id, 'item_count', $count_value );
    $item_count = get_post_meta( $post_id, 'item_count', true );
    return $item_count;
  } else {
    $rand_item_count = mt_rand(20, 40);
    add_post_meta( $post_id, 'item_count', $rand_item_count, true);
    $item_count = get_post_meta( $post_id, 'item_count', true );
    return $item_count;
  }
}
